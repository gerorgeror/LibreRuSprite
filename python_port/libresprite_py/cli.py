...
from libresprite_py.filters import ChannelTarget, invert_sprite
...
def _parse_channels(value: str) -> ChannelTarget:
    normalized = value.lower().replace(" ", "")
    allowed = {"r", "g", "b", "a"}
    unknown = set(normalized) - allowed
    if unknown:
        raise argparse.ArgumentTypeError(f"unknown channels: {''.join(sorted(unknown))}")
    return ChannelTarget(
        red="r" in normalized,
        green="g" in normalized,
        blue="b" in normalized,
        alpha="a" in normalized,
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    invert_cmd = sub.add_parser("invert", help="Apply invert color filter to sprite project")
    invert_cmd.add_argument("--project", required=True)
    invert_cmd.add_argument(
        "--channels",
        type=_parse_channels,
        default=ChannelTarget(red=True, green=True, blue=True, alpha=False),
        help="Any subset of rgba, default=rgb",
    )
...
def _run_invert(args: argparse.Namespace) -> int:
    sprite = Sprite.load_json(args.project)
    filtered = invert_sprite(sprite, args.channels)
    filtered.save_json(args.project)
    print(f"inverted {args.project}")
    return 0
